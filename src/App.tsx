...
function App() {
  ...
  async function createOrg(name: string) {
    try { await invoke("create_org", { name }); setActiveOrg(name); setOrgs([{name}]); setScreen("dashboard"); }
    catch (e) { setError(String(e)); }
  }

  async function shareOrg() {
    if (!activeOrg) return;
    try { const t: string[] = await invoke("share_org", { org: activeOrg }); setTickets(t); }
    catch (e) { setError(String(e)); }
  }
    ...
    <div style={{ padding: 20, fontFamily: "system-ui" }}>
      ...
      {tickets.length > 0 && (
        <div style={{ background: "#fff3cd", padding: 12, marginBottom: 16, borderRadius: 4 }}>
          ...
          {tickets.map((t, i) => (
            <div key={i} style={{ fontFamily: "monospace", fontSize: 12, wordBreak: "break-all", marginTop: 4 }}>{t.slice(0, 80)}...</div>
          ))}
...
function Dashboard({ org }: { org: string }) {
  ...
  return <div><h2>Dashboard — {org}</h2><p>Network: {status || "checking..."}</p><p>Devices: {devices.length}</p></div>;
}
...
function DevicesScreen({ org }: { org: string }) {
  ...
  async function load() { const d: any[] = await invoke("list_devices", { org }); setDevices(d); }
  useEffect(() => { load(); }, [org]);

  async function add() {
    await invoke("add_device", { org, nodeId, name, person, role });
    setNodeId(""); setName(""); setPerson(""); setRole("sales"); load();
  }
  async function toggleActive(id: string, active: boolean) {
    await invoke("update_device", { org, nodeId: id, active: !active, role: null }); load();
  }
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        ...
        <tbody>{devices.map(d => (
          <tr key={d.node_id} style={{ opacity: d.active ? 1 : 0.4 }}>
            ...
            <td style={{padding:4}}>{d.name}</td><td style={{padding:4}}>{d.person}</td><td style={{padding:4}}>{d.role}</td>
...
function RolesScreen({ org }: { org: string }) {
  ...
  useEffect(() => { invoke<any[]>("list_roles", { org }).then(setRoles).catch(() => {}); }, [org]);
  return (
    <div>
      <h2>Roles — {org}</h2>
      {roles.length === 0 && <p>No roles defined. Default: admin has full access.</p>}
      {roles.map(r => <div key={r.name} style={{marginBottom:12}}><strong>{r.name}</strong><div>can_open: {JSON.stringify(r.can_open)}</div><div>can_write: {JSON.stringify(r.can_write)}</div></div>)}
    </div>
  );
}
